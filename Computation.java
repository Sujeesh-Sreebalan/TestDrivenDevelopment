...
class Computation{
	...
	public int calculateLongestMirrorSize(int[] maxMirror) throws AssertionError{
		...
		for(int startIndex = 0; startIndex < size; startIndex++) {
			...
			for(int endIndex = size-1; startIndex + count < size && endIndex > -1; endIndex--) {
				// if Left index(startIndex) value is equal to Right (endIndex) value then increasing the count
	...
	public int calculateClumps(int[] countClumps) throws AssertionError{
		...
		if(size > 1) {
			for(int index = 1; index < size-1; index++) {
				// checking for condition if previous elemenet and current element should be same 
				// and current element and next element should be differnet.
				if(countClumps[index-1] == countClumps[index] && countClumps[index] != countClumps[index+1]) {
					clumps++;
				}
			}
			// if last and second last element is same
		...
		for(int xIndex = 0; xIndex < size; xIndex++) {
			if(fixXY[xIndex] == X) {
				for(int yIndex = 0; yIndex < size; yIndex++) {
					if(fixXY[yIndex] == Y) {
						// checking for if previous index of Yindex is not X and swapping the elements
	...
	public int calculateSum(int[] splitArray) throws AssertionError{
		...
		// initially taking arrLeftSum as first element and arrRightSum as sum of array minus first element 
